import type { Metadata } from 'next'

const legalLinks = [
  { label: 'Politique de confidentialité', href: '/fr/privacy-policy' },
  { label: 'Informations fiscales', href: '/fr/tax-information' },
  { label: "Conditions d'utilisation", href: '/fr/terms-of-use' },
]

export const metadata: Metadata = {
  title: "Conditions d'utilisation | CUSI Flowers",
  description:
    "Conditions générales d'utilisation du site CUSI Flowers : commandes, livraisons, disponibilité et responsabilité.",
  alternates: {
    canonical: 'https://www.cusiflores.com/fr/terms-of-use',
  },
  robots: {
    index: true,
    follow: true,
  },
}

export default function TermsOfUsePageFr() {
  return (
    <div className="min-h-screen bg-[#f6efe9] text-[#241715]">
      <header className="sticky top-0 z-40 border-b border-[#ead8cf] bg-[#fffaf7]/90 backdrop-blur-md">
        <div className="mx-auto flex w-[92%] max-w-7xl items-center justify-between py-4">
          <a href="/fr" className="text-sm font-semibold tracking-[0.16em] text-[#2a1c19]">CUSI</a>
          <div className="flex items-center gap-2">
            <div className="inline-flex items-center gap-2 rounded-full border border-[#d8c7be] bg-white px-3 py-1 text-xs font-semibold text-[#2e231f]">
              <a href="/condiciones-generales-de-uso" className="underline underline-offset-2 hover:opacity-75">ES</a>
              <span className="text-[#b8a9a2]">|</span>
              <a href="/en/terms-of-use" className="underline underline-offset-2 hover:opacity-75">EN</a>
              <span className="text-[#b8a9a2]">|</span>
              <a href="/it/terms-of-use" className="underline underline-offset-2 hover:opacity-75">IT</a>
              <span className="text-[#b8a9a2]">|</span>
              <span>FR</span>
            </div>
            <a href="/fr" className="rounded-full border border-[#d8c7be] px-4 py-2 text-xs font-semibold text-[#2e231f]">Retour à l'accueil</a>
          </div>
        </div>
      </header>

      <main className="mx-auto w-[92%] max-w-5xl py-8 md:py-12">
        <section className="rounded-3xl border border-[#ead8cf] bg-white p-6 md:p-10">
          <h1 className="text-3xl leading-tight md:text-5xl">Conditions d'utilisation</h1>
          <p className="mt-4 text-sm text-[#6f5851]">Dernière mise à jour : avril 2026</p>
          <p className="mt-4 text-base leading-8 text-[#5d4740]">
            En naviguant sur ce site, en demandant des informations ou en passant une commande chez CUSI, vous acceptez ces conditions.
          </p>
        </section>

        <section className="mt-6 space-y-6 rounded-3xl border border-[#ead8cf] bg-white p-6 md:p-10">
          <article>
            <h2 className="text-2xl md:text-3xl">1. Objet du site</h2>
            <p className="mt-3 text-base leading-8 text-[#5d4740]">
              Ce site présente les produits CUSI, nos services floraux, promotions, moyens de contact et options de livraison.
            </p>
          </article>

          <article>
            <h2 className="text-2xl md:text-3xl">2. Disponibilité des produits</h2>
            <p className="mt-3 text-base leading-8 text-[#5d4740]">
              Les images de fleurs sont illustratives. Les compositions finales peuvent varier selon la saison et le stock disponible.
            </p>
          </article>

          <article>
            <h2 className="text-2xl md:text-3xl">3. Commandes et confirmations</h2>
            <p className="mt-3 text-base leading-8 text-[#5d4740]">
              Une commande est confirmée après validation des informations client, de la disponibilité, des conditions de livraison et du paiement.
            </p>
          </article>

          <article>
            <h2 className="text-2xl md:text-3xl">4. Livraisons</h2>
            <p className="mt-3 text-base leading-8 text-[#5d4740]">
              Les livraisons dépendent de la zone desservie, des créneaux disponibles, du trafic et de l'exactitude des informations fournies.
            </p>
          </article>

          <article>
            <h2 className="text-2xl md:text-3xl">5. Contact</h2>
            <p className="mt-3 text-base leading-8 text-[#5d4740]">Téléphone : +52 1 55 2109 2665</p>
            <p className="text-base leading-8 text-[#5d4740]">Email : contacto@cusiflores.com</p>
          </article>
        </section>
      </main>

      <footer className="mx-auto w-[92%] max-w-7xl border-t border-[#e6d5cc] py-8 text-sm text-[#6f5851]">
        <p className="text-xs font-semibold uppercase tracking-[0.14em] text-[#6f5851]">Informations légales</p>
        <div className="mt-2 flex flex-col gap-1.5 text-sm font-semibold">
          {legalLinks.map((link) => (
            <a key={link.href} href={link.href} className="underline underline-offset-2 transition hover:opacity-70">
              {link.label}
            </a>
          ))}
        </div>
      </footer>
    </div>
  )
}
